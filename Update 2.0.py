...
game_starter = None             # User or Computer
selected_string_length = None   # Number from 15 to 25
Get_Started = None  # Define Get_Started as a global variable
numbers = []
game_scores = [0, 0]
current_turn = 0
selected_index = [None]  # Initialize selected_index
selected_pair = None     # Initialize selected_pair
...
def Game_Window():
    ...
    if selected_algorithm is None or game_starter is None or selected_string_length is None:
        messagebox.showerror("Error", "Please make sure to select the game starter, algorithm, and string length before starting the game!")
    else:
        ...
        # Window Title
        Gen_Str_Label = Label(Game, text="The Generated String is: ", font=("Helvetica", 18)).pack(pady=20)
        
        
        # Function to Generate the string
        def generate_str(selected_string_length):
            return [random.randint(1, 9) for _ in range(selected_string_length)]        
    
        numbers = generate_str(selected_string_length)
        Gen_Str_Label_1 = Label(Game, text=numbers, font=("Helvetica", 18), bootstyle="default,inverse").pack(pady=10)
    
        # Label to display selected pair
        Selection_label = Label(Game, text="Please select any two adjacent numbers", font=("Helvetica", 14)).pack(pady=10)
        ...
        def select_number(index):
            ...
            if selected_index[0] is None:
                ...
                buttons[index].config(bg="yellow")  # Highlight the selected button
                ...
                # Clear selection
                buttons[selected_index[0]].config(bg="")
        ...
        # Determine button width based on the length of the generated string
        button_width = max(2, 3)
        
        # Create buttons for each number in the string
        buttons = []
        for i, number in enumerate(numbers):
            button = Button(button_frame, text=number, width=button_width, command=lambda idx=i: select_number(idx), bootstyle="warning-outline")
        ...
        def check_game_end():
            ...
            if len(numbers) <= 1:
                ...
                if response:
                    Back_To_Get_Started()              # Start a new game
                else:
                    Game.destroy()      # Close the game window
        ...
        def replace_pair():
            ...
            del numbers[selected_index[0] + 1]
        ...
        def AI_turn():
            global numbers, game_scores, current_turn, selected_algorithm, selected_index, selected_pair, nodes_visited
            ...
            if best_action is not None:
                ...
                print(f"Time taken for this move: {move_time} seconds")
        ...
        Play_button = Button(Play_frame, text="Play", command=Player_turn, bootstyle="success", width=10)  # Set width to 10
        ...
        Start_button = Button(Play_frame, text="Start", command=Start_game, bootstyle="success", width=10)  # Set width to 10
        Start_button.pack(padx=100, pady=10) 
        
        Reset_button = Button(Play_frame, text="Reset", bootstyle="danger", command=reset_pairs, width=10)  # Set width to 10
        ...
        AI_Title_Score = Label(Score_Title_frame, text="AI SCORE ", font=("Helvetica", 14)).pack(side=LEFT, padx=200, pady=10)
        
        Your_Title_Score = Label(Score_Title_frame, text="YOUR SCORE ", font=("Helvetica", 14)).pack(side=LEFT, padx=200, pady=10)
        ...
        Back_button = Button(Game, text="Back", bootstyle="danger", command=Back_To_Get_Started, width=10)  # Set width to 10
...
def Get_Started_Window():
    ...
    # Create a Get_Started_Window
    Get_Started = Toplevel()
    Get_Started.title("Numerical String Game")
    Get_Started.state('zoomed')  # Maximize the new window

    # Window Title
    Get_started_Label = Label(Get_Started, text="Game Setup", font=("Helvetica", 28)).pack(pady=40)

    # Who starts the game?     
    Who_Start_Label = Label(Get_Started, text="Who starts the game?", font=("Helvetica", 14)).pack(pady=10)
    ...
    for Who_Start in Game_Starters:
        Radiobutton(frame_2, bootstyle="success", variable=Game_Starter, text=Who_Start, value=Who_Start, command=Starter_selc).pack(side=LEFT, padx=10)

    Starter_Label = Label(Get_Started, text="The game will be started with the ", font=("Helvetica", 9))
    Starter_Label.pack(pady=10)

    # Choosing the algorithm!     
    Algorthim_Label = Label(Get_Started, text="Choose the algorithm!", font=("Helvetica", 14)).pack(pady=10)
    ...
    for Algorithm in Algorithms:
        Radiobutton(frame_2, bootstyle="success", variable=My_Algorithm, text=Algorithm, value=Algorithm, command=Algorithm_selc).pack(side=LEFT, padx=10)

    Algorithm_Label = Label(Get_Started, text="The chosen algorithm is ", font=("Helvetica", 9))
    Algorithm_Label.pack(pady=10)

    # String length     
    Str_Length_Label = Label(Get_Started, text="Select the string length!", font=("Helvetica", 14)).pack(pady=10)
...
def Main_Window():
    ...
    # Title
    label_1 = Label(text="Welcome to the Numerical String Game!", font=("Helvetica", 28)).pack(pady=50)
    # Subtitle
    label_2 = Label(root, text="Game description:", font=("Helvetica", 18), bootstyle="default,inverse").pack(pady=30)
    # Descritption
    label_3 = Label(text="Before starting the game, the player should choose the length of the numerical string which is should be between 15 and 25. Then, the game software randomly generates a numerical string containing the numbers 1 to 9. At the beginning of the game, Each player has 0 points and the players perform the moves sequentially. The player chooses any pair of two adjacent numbers and replaces it with a single number based on the following conditions:", font=("Helvetica", 14))
    label_3.pack()
    label_3.configure(wraplength=1200)
    
    label_4 = Label(text="1. If the sum of the numbers > 7, then the pair of numbers is replaced by 1 and one point is added to the player's score.", font=("Helvetica", 14))
    label_4.pack()
    label_4.configure(wraplength=1000)
    
    label_5 = Label(text="2. If the sum of the numbers < 7, then it is replaced by 3 and 1 point is deducted from the opponent's score.", font=("Helvetica", 14))
    label_5.pack()
    label_5.configure(wraplength=1000)
    
    label_6 = Label(text="3. if the sum of the numbers = 7, then it is replaced by 2 and 1 point is added to both the player's score and the opponent's score.",font=("Helvetica", 14))
    label_6.pack()
    label_6.configure(wraplength=1000)
    
    label_7 = Label(text="Finally, The game ends when there are no more pairs of numbers to replace. If the players have the same number of points, the result is a draw. Otherwise, the player with the higher number of points wins. ", font=("Helvetica", 14))
